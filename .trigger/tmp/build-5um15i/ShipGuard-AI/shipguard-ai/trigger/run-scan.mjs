...
// lib/ai/analyzer.ts
init_esm();
function parseTruffleHog(trufflehog) {
  const issues = [];
  const findings = Array.isArray(trufflehog) ? trufflehog : trufflehog && typeof trufflehog === "object" && "results" in trufflehog ? trufflehog.results : [];
...
__name(parseTruffleHog, "parseTruffleHog");
function parseOSV(osv) {
  const issues = [];
  const vulnerabilities = osv && typeof osv === "object" && "vulnerabilities" in osv ? osv.vulnerabilities : [];
...
__name(parseOSV, "parseOSV");
function parseSemgrep(semgrep) {
  const issues = [];
  const results = semgrep && typeof semgrep === "object" && "results" in semgrep ? semgrep.results : [];
...
__name(parseSemgrep, "parseSemgrep");
function parseReactDoctor(reactDoctor) {
  const issues = [];
  const data = reactDoctor;
  if (!data.issues) return issues;
  for (const issue of data.issues) {
    const category = issue.type?.includes("error") || issue.type?.includes("warning") ? "scalability" : "distribution";
    issues.push({
      guard: category,
      category,
      severity: "medium",
...
__name(parseReactDoctor, "parseReactDoctor");
function parseFileChecks(fileChecks) {
  const issues = [];
  ...
  const hasAnalytics = fileChecks.has_plausible === "true" || fileChecks.has_google_analytics === "true" || fileChecks.has_posthog === "true";
  if (!hasAnalytics) {
    issues.push({
      guard: "distribution",
      category: "analytics",
...
__name(parseFileChecks, "parseFileChecks");
async function analyzeToolOutputs(outputs) {
  const allIssues = [];
...
__name(analyzeToolOutputs, "analyzeToolOutputs");
function calculateScores(issues) {
  ...
  scores.overall = Math.round(
    scores.security * 0.35 + scores.scalability * 0.25 + scores.monetization * 0.2 + scores.distribution * 0.2
  );
  return scores;
}
...
async function runTool(cmd, cwd) {
  try {
...
var runScanTask = task({
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      const cloneUrl = githubToken ? `https://x-access-token:${githubToken}@github.com/${repoOwner}/${repoName}.git` : `https://github.com/${repoOwner}/${repoName}.git`;
      await execAsync(
        `git clone --depth 1 --branch ${branch} ${cloneUrl} .`,
        { cwd: cloneDir, timeout: 6e4 }
      );
      ...
      const lockfile = hasPackageLock ? "package-lock.json" : hasYarnLock ? "yarn.lock" : hasPnpmLock ? "pnpm-lock.yaml" : null;
      ...
      let trufflehog = [];
      if (trufflehogRaw) {
        trufflehog = trufflehogRaw.split("\n").filter(Boolean).map((line) => {
          try {
      ...
      const semgrepResults = semgrep.results ?? [];
      ...
      const check = /* @__PURE__ */ __name(async (relPath) => fileExists(join(cloneDir, relPath)), "check");
      ...
      if (issues.length > 0) {
        ...
        const { error: insertError } = await supabase.from("issues").insert(issuesToInsert);
        if (insertError) {
...
//# sourceMappingURL=run-scan.mjs.map
